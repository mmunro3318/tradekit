...
- **Addendum (same session):** it is every C-style escape, not just `\n` — a Windows path `scripts\register_…` in a heredoc landed as `scripts<CR>egister_…` (committed, repaired in 877b011) and `D:\tradekit-data` as `D:<TAB>radekit-data`. The `D:<TAB>radekit-data` typos already sitting in docs/ROADMAP.md (line ~289) and the docs/ARCHIVE-README.md header comment are the same defect from earlier sessions — left as found. Cheap post-edit check: scan the file for lone CR and TAB bytes. Promoted to memory: bash-heredoc-unescapes-backslashes.
